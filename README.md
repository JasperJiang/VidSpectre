...
- **类型筛选**：订阅列表支持按全部/电影/电视剧筛选
- **集数追踪**：标记当前看到第几集，支持快速已看按钮一键+1
- **关键字过滤**：按资源质量筛选（2160p、HDR、杜比等）
- **定时检查**：按全局周期自动检测所有订阅更新（可开关）
- **手动触发**：订阅列表顶部可随时手动触发爬取
- **磁力链接获取**：一键获取下载链接
...
| GET | `/api/subscriptions/<id>/movie-links` | 获取电影资源列表 |
